...
## 자동 신청은 넣지 않았다
수강신청 시스템에는 **`자동 등록 방지` 캡차**가 있다 — 퀴즈의 정답을 클릭해야 하고
**3회 오답 시 자동 로그아웃**된다 (`/lecapply/lecapply/captcha/init`,
`LayerPopup.captcha`). 학교가 매크로를 막기 위해 둔 봇 차단 장치이므로 우회하지 않는다.
자동 신청 대신 **부전공 자리가 나면 즉시 신청할 수 있는 정보를 담은 긴급 알림**을 보낸다.

참고로 신청 자체는 `POST /lecapply/lecapply/saveLecapplyServlet`
(`SUBJ_NO`, `CLASS_NO` 등)이며, 수강정정 기간은 화면에 표시된다
(2026학년도 2학기: 2026-09-01 08:00 ~ 2026-09-07 17:00).

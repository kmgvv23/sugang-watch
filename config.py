...
# 확정 과목과 시간이 겹쳐 실제로는 옮길 수 없는 분반 (알림에 경고를 붙인다)
CONFLICT = {"004": "데이터베이스(CB2001111-059) 월·수 13:30 과 겹침"}

# 내 트랙에 자리가 나면 수강정정 시스템에 로그인해 분반변경을 자동 시도한다.
# False 로 두면 알림만 보낸다.
AUTO_CHANGE = False   # 001분반 보유 중 — 알림만 받고 변경은 직접 판단한다

# 시도 순서 (앞부터 시도). 현재 보유 분반보다 순위가 높은 분반에만 변경을 시도한다.
# 001 월수 10:30 강상훈 / 002 화목 15:00 김진우 / 003 월수 12:00 박희진
# 세 분반 모두 시간표 충돌이 없고 선호도 없으므로, 먼저 열린 것을 잡는다.
PREFERENCE = ["002", "003", "004"]

# 목표 분반을 하나 잡으면 감시를 끝낸다 (더 바꾸려 들지 않는다).
STOP_AFTER_SUCCESS = True

# 현재 보유 분반 (참고용, 실제 값은 실행 시 시스템에서 읽는다)
CURRENT_CLASS_HINT = "001"

CHANGE_COOLDOWN_SEC = 90   # 같은 분반 재시도 최소 간격
CHANGE_MAX_TRIES = 3       # 같은 분반 최대 시도 횟수

# --- 감시 공백 감지 ---
# 이 시간 이상 조회가 끊겼다가 재개되면 복구 직후 텔레그램으로 보고한다.
# (끊긴 동안에는 네트워크가 없어 알림 자체를 보낼 수 없으므로 '사후 보고'다)
OUTAGE_ALERT_SEC = 300

# 정상 작동 중임을 주기적으로 알린다. 이게 안 오면 뭔가 잘못된 것.
# 0 으로 두면 끈다.
HEARTBEAT_SEC = 6 * 3600

# 이 시간 동안 조회가 한 번도 성공하지 못하면 브라우저를 통째로 새로 띄운다.
# (페이지 재적재만으로는 망가진 브라우저 세션에서 복구되지 않는다)
FORCE_RESTART_SEC = 240
